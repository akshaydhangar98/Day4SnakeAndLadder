...
public class UC7_TwoPlayerGame {
	...
	public static void main(String[] args) {
			...
			int randomCheck, randomPlay, randomCheck1, randomPlay1; 
			...
			while(position1 < destination && position2 < destination)
			{
				diceRoll1++;
					...
					randomPlay = (int) (Math.random() * 10) % 3;
					switch(randomPlay) {
					case 0:
					...
					diceRoll2++;
					randomCheck1 = (int) Math.floor(Math.random() * 10) % 6+1;
					randomPlay1 = (int) (Math.random() * 10) % 3;
					switch(randomPlay1) {
					case 0:
						System.out.println("No play");
						position2+=0;
